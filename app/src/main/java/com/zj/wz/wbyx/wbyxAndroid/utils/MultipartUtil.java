...
public class MultipartUtil {
    ...
    public static Map<String, RequestBody> getRequestBodyMap(Map<String, ?> parms, String img, File[] imgFile) {
        final Map<String, RequestBody> body = new HashMap<>();
        if (parms != null) {
        ...
        if (imgFile != null) {
                if (imgFile != null) {
                    for (int i = 0, size = imgFile.length; i < size; i++) {
                        final File file = imgFile[i];
                        final MediaType mediaType = MultipartUtil.getFileMediaType(file.getAbsolutePath());
                        final RequestBody fileBody = RequestBody.create(mediaType, file);
                        body.put(img + "\"; filename=\"" + file.getName(), fileBody);
                    }
                }
            }
        return body;
