...
/**
 * @ClassName GoodsAnimUtil
 * @Description TODO 购物车添加抛物线动画
...
public abstract class GoodsAnimUtil {
    ...
    private static View mImgcar = null;
    ...
    public static void setAnim(Activity activity, View imgphoto, View imgcar) {
        ...
        // 设置buyImg的图片
        buyImg.setImageResource(R.drawable.icon_dian_dian);
        // 开始执行动画
        startAnim(buyImg, start_location1);
    }

    public static void setAnim(Activity activity, View imgphoto) {
        mActivity = activity;
        // 一个整型数组，用来存储按钮的在屏幕的X、Y坐标
        int[] start_location = new int[2];
        // 这是获取购买按钮的在屏幕的X、Y坐标（这也是动画开始的坐标）
